...
export function healingTargets(currentActor, worldActors, scenes = []) {
  const targets = new Map();
  const add = (actor, label, key = actor?.uuid ?? actor?.id) => {
    if (actor?.type !== "character" || !actor.isOwner || targets.has(key)) return;
    targets.set(key, { actor, label });
  };
  for (const actor of worldActors) add(actor, `${actor.name} — katalog aktorów`);
  for (const scene of scenes) {
    for (const token of scene.tokens ?? []) {
      if (!token.actorLink) add(token.actor, `${token.name} — token na scenie „${scene.name}”`, token.actor?.uuid ?? token.uuid);
    }
  }
  // Karta może należeć do niepołączonego tokena, także poza aktywną sceną.
  add(currentActor, `${currentActor.name} — otwarta karta`);
...
export async function healOverTime(actor, group = false) {
  ...
  busy = true;
  try {
    const targets = group ? healingTargets(actor, [...game.actors], [...(game.scenes ?? [])]) : [{ actor, label: actor.name }];
    ...
    const data = await foundry.applications.api.DialogV2.input({
      ...
      content: `<label>Liczba dni<input type="number" name="days" value="1" min="1" max="3650" step="1"></label>
        ...
        ${group ? "<p>Token niepołączony z katalogiem ma własne rany. Wybierz właściwy wpis; otwarta karta jest zaznaczona domyślnie.</p>" : ""}
        ${actors.map((a, i) => `<p><label><input type="checkbox" name="actor${i}" ${a === actor || (a.uuid && a.uuid === actor.uuid) ? "checked" : ""}>${esc(targets[i].label)} — ran z karą: ${[...a.items].filter(item => item.type === "injury" && item.system.penaltyPercent > 0).length}</label>
    ...
    const confirmed = await foundry.applications.api.DialogV2.confirm({
      ...
      content: `<div style="max-height:55vh;overflow:auto">${plans.map(p => `<h3>${esc(targets.find(target => target.actor === p.actor).label)}</h3>${p.injuries.map(({ item, plan }) => `<p>${esc(item.name)}: ${plan.before}% → ${plan.after}%; ${esc(plan.mode)}, dni: ${plan.days}. Zapas dnia bez odpoczynku: ${plan.pendingDay}.</p>`).join("")}`).join("")}</div>`,
